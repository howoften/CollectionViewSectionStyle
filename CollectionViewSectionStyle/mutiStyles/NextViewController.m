...
@interface NextViewController ()<UICollectionViewDataSource,UICollectionViewDelegate,UICollectionViewSectionDecorationFlowLayout>
...
- (void)initialization{
    _myCollectionView = ({
        LLCollectionSectionDecorationLayout *layout = [[LLCollectionSectionDecorationLayout alloc]init];
        ...
        if (!self.isRoundWithHeaerView && !self.isRoundWithFooterView) {
            layout.decorateAreaKind = LLCollectionViewSectionDecorateAreaCell;
        }else if (self.isRoundWithHeaerView && !self.isRoundWithFooterView) {
            layout.decorateAreaKind = LLCollectionViewSectionDecorateAreaHeaderAndCell;
        }else if (!self.isRoundWithHeaerView && self.isRoundWithFooterView) {
            layout.decorateAreaKind = LLCollectionViewSectionDecorateAreaFooterAndCell;
        }else if (self.isRoundWithHeaerView && self.isRoundWithFooterView) {
            layout.decorateAreaKind = LLCollectionViewSectionDecorateAreaAll;
...
- (LLCollectionSectionDecorationModel *)collectionView:(UICollectionView *)collectionView layout:(LLCollectionSectionDecorationLayout*)collectionViewLayout configModelForSection:(NSInteger)section{
    LLCollectionSectionDecorationModel *model = [[LLCollectionSectionDecorationModel alloc] init];
//    model.decorateAreaKind = collectionViewLayout.decorateAreaKind;
    model.cornerRadius = 10;
//    model.decorateAreaPadding = UIEdgeInsetsMake(10, 10, 10, 10);
